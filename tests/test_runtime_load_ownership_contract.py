...
def test_declared_business_owners_load_before_runtime_contracts():
    ...
    assert ip_owner < onboarding_contract
    assert "ip-runtime-contract-v1.js" not in scripts
    assert "script-recommendation-navigation-fix.js" not in scripts
...
def test_owner_modules_declare_their_canonical_scope():
    ...
    assert "推荐昵称唯一 Owner" in NICKNAME_CORE
...
def test_phase_two_runtime_governance_documents_are_committed():
    ...
    assert "Production 未被修改" in MIGRATION_PLAN

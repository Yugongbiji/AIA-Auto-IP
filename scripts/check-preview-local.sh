...
if ! "$PY" -m pytest --version >/dev/null 2>&1; then
  fail "Preview .venv 尚未安装 pytest。执行：.venv/bin/pip install pytest"
fi
"$PY" -m pytest tests/test_ip_policy_contract.py -q

log "2/4 检查 index.html 实际加载的 JavaScript 语法"
if ! command -v node >/dev/null 2>&1; then
  fail "未找到 node，无法执行前端语法门禁。"
fi

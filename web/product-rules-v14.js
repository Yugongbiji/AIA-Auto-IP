...
(function () {
  ...
  insertBeforeDepartment({
    key: 'previousCareer',
    label: '过往职业/经历',
    ask: '你过去做过什么工作，或有哪些比较有代表性的长期经历？这类真实积累可以成为内容支线。已有资料里有的话我不会重复问。',
  ...
  const HOBBY_RULES = [
    ...
    [/旅行|旅游/, '旅行'], [/摄影/, '摄影'], [/读书|阅读/, '读书'], [/美食|烹饪|做饭/, '美食'], [/育儿|亲子/, '育儿升学'],
  ];
  ...
  function addSourceLabels(content) {
    if (!content) return;
    const strategy = window.buildIpContentStrategy?.(state.profile || {}, state.proposals?.[0]?.proposal || {});
  ...
  if (typeof renderProposal === 'function') {
    ...
    renderProposal = function renderProposalV14(proposal, version) {
      ...
      addSourceLabels(document.getElementById('proposal-content'));
      return result;
    };
  }
})();

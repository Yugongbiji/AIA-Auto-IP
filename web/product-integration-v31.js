...
(function(){
  ...
  // 62：两个入口统一为组件库式“图标 + 两行文字”；IP方案不显示版本号。
  ...
  function fixFloatButtons(){
    ...
    if(p){p.classList.add('aia-v31-float');p.innerHTML=`<span class="aia-float-icon">${icon('profile')}</span><span class="aia-float-copy"><b>我的</b><b>资料</b></span>`;p.setAttribute('aria-label','我的资料');}
    if(s){s.classList.add('aia-v31-float');s.innerHTML=`<span class="aia-float-icon">${icon('proposal')}</span><span class="aia-float-copy"><b>IP</b><b>方案</b></span>`;s.setAttribute('aria-label','IP方案');}
  }

  // 67：简介先做“行级完整性”检查。真实素材也不能机械拼积木。
  ...
  // 69：一句话IP定位不是标签堆砌，只保留“人物称呼 + 最强记忆点 + 可选内容价值”。
  ...
  function enforceHeadline(proposal){if(!proposal)return;proposal.headline=safeHeadline(state.profile||{});}

  // 64/66：问号回到标题同行；删除无效长提示。
  function fixProposalDom(){
    ...
    [...root.querySelectorAll('h2,h3,strong')].forEach(h=>{
      ...
      const section=h.closest('section,article,.proposal-section,.proposal-card,.proposal-block')||h.parentElement;
      if(!section)return;
  ...
  new MutationObserver(fixFloatButtons).observe(document.body,{childList:true,subtree:true});fixFloatButtons();
  ...
  .ip-floating-button.aia-v31-float{width:72px!important;min-width:72px!important;height:56px!important;min-height:56px!important;padding:0 9px!important;border-radius:16px!important;display:flex!important;flex-direction:row!important;gap:7px!important;box-shadow:0 8px 22px rgba(60,7,18,.16)!important;transition:transform .15s ease,box-shadow .15s ease!important}
  ...
  .aia-float-icon{display:flex!important;width:20px!important;height:20px!important;flex:0 0 20px!important;position:static!important;overflow:visible!important;clip:auto!important;margin:0!important}.aia-float-icon svg{width:20px;height:20px;fill:none;stroke:currentColor;stroke-width:1.8;stroke-linecap:round;stroke-linejoin:round}.aia-float-copy{display:flex!important;flex-direction:column!important;gap:1px!important;position:static!important;width:auto!important;height:auto!important;overflow:visible!important;clip:auto!important;margin:0!important}.aia-float-copy b{font-size:11px;line-height:1.05;white-space:nowrap}

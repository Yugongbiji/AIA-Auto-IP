...
(function(){
  ...
  const DECORATIVE_SYMBOLS=/[卍卐★☆✦✧✪✿❀♡♥ღ]/g;
  ...
  function cleanNicknameDisplay(name){return t(name).replace(/\p{Extended_Pictographic}|\uFE0F/gu,'').replace(DECORATIVE_SYMBOLS,'').replace(/[._。·•~～!！?？]+$/g,'').trim();}
  function normalizeSearchable(name){return cleanNicknameDisplay(name).replace(/\s+/g,'').replace(/[^\u4e00-\u9fa5A-Za-z0-9]/g,'');}
  function fullEnglish(name){const n=normalizeSearchable(name);return !!n&&/^[A-Za-z0-9]+$/.test(n)&&/[A-Za-z]/.test(n)&&!/[\u4e00-\u9fa5]/.test(n)&&!/^[0-9]+$/.test(n);}

  function peerAnchors(p){return (p.peerReviewSummary?.topNicknames||[]).filter(x=>!missing(x?.label)).sort((a,b)=>Number(b?.count||1)-Number(a?.count||1)).map(x=>cleanNicknameDisplay(x.label));}
  function preferredAnchors(p){const v=cleanNicknameDisplay(p.preferredName);return v&&!missing(v)?[v]:[];}
  function existingPersonAnchors(p){const out=[];[p.videoNickname,p.xiaohongshuNickname].map(cleanNicknameDisplay).filter(v=>v&&!missing(v)&&!BANNED.test(v)).forEach(value=>{const matches=value.match(/(?:小[\u4e00-\u9fa5]{1,2}|阿[\u4e00-\u9fa5]{1,2}|[\u4e00-\u9fa5]{1,3}(?:姐|哥|老师|妈妈|妈|爸爸|爸|总)|[A-Za-z]{2,12}(?:姐|哥)?)/g)||[];out.push(...matches);});return uniq(out);}
  ...
  function approvedPresetOptions(profile){const preset=profile?.nicknamePreset;if(!preset||t(preset.status)!=='approved')return [];const alternatives=Array.isArray(preset.alternatives)?preset.alternatives:(Array.isArray(preset.candidates)?preset.candidates:[]);const names=uniq([t(preset.primary),...alternatives.map(t)]).map(cleanNicknameDisplay).filter(name=>name&&!BANNED.test(name)&&hasChinese(name)&&name.length<=18);return uniq(names).map((name,index)=>({name,angle:index===0?'首选推荐':'备选推荐',reason:presetReason(name,profile),memoryKind:'preset'}));}
  ...
  window.aiaNicknamePolicyV1=Object.freeze({controlledOptions,enforce,BANNED,anchors,pickAnchor,aiFallbackOptions,naturalNameAnchors,awkward,distinctiveOptions,memorablePeerDescriptor,descriptorOptions,normalizeSearchable,cleanNicknameDisplay,fullEnglish,memoryScore,rankByMemory,approvedPresetOptions,presetReason,reasonFor});
})();

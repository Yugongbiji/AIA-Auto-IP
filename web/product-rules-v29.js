...
(function () {
  ...
  const cleanNickname=value=>window.aiaNicknamePolicyV1?.cleanNicknameDisplay?.(value)||text(value).replace(/\p{Extended_Pictographic}|\uFE0F/gu,'').replace(/[卍卐★☆✦✧✪✿❀♡♥ღ]/g,'').replace(/[._。·•~～!！?？]+$/g,'').trim();

  function knownAddresses(profile){const items=[];(profile?.peerReviewSummary?.topNicknames||[]).forEach(item=>{const label=cleanNickname(item?.label);if(label&&!missing(label))items.push(label);});const preferred=cleanNickname(profile?.preferredName);if(preferred&&!missing(preferred))items.push(preferred);const name=text(profile?.name);if(name){if(name.length>=3)items.push(name.slice(-2));items.push(name);}return uniq(items).filter(Boolean);}
  function evaluateNickname(name,profile){
    const value=text(name),cleaned=cleanNickname(value),issues=[],strengths=[];if(!value||missing(value))return{name:value,cleaned:value,issues:['当前没有填写昵称'],strengths:[],hasPersonAnchor:false,missing:true};
    if(cleaned!==value)issues.push(`包含特殊符号、Emoji 或装饰标点，推荐版本应简化为“${cleaned}”`);
    if(cleaned.length<=10)strengths.push('长度比较利落，容易记');else if(cleaned.length>14)issues.push('昵称偏长，不容易一次记住');
    if(/保险|友邦|\bAIA\b/i.test(cleaned))issues.push('包含当前推荐规则中的禁用业务/品牌词');
    if(/[©®™]|https?:\/\/|www\.|微信|vx|V信|电话|手机号/i.test(value))issues.push('包含联系方式、链接或导流信息');
    if(/^[A-Za-z0-9]+$/.test(cleaned)&&/[A-Za-z]/.test(cleaned)&&!/[\u4e00-\u9fa5]/.test(cleaned))issues.push('纯英文昵称对普通中文用户的记忆、输入和搜索成本较高，原则上只作兜底');
    const matched=knownAddresses(profile).filter(address=>address&&cleaned.includes(address));

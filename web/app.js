...
const creativeState = {
  script: { started: false, messages: [], source: '', generating: false },
...
function handoffXhsToScript(source) {
  if (!source) return;
  addCreativeMessage('xhs', '已确认修改风险表达，正在转入脚本改写。', 'user');
  ...
  runScriptRewrite(source, '请根据小红书排版后的风险提示，修改有风险的表达；保留原文核心观点与事实。');
}
...
function startWorkspace(profile, matched, history = [], proposals = [], planningHistory = [], contentPlans = [], creativeHistory = []) {
  ...
  ['script', 'xhs'].forEach((tool) => { creativeState[tool].started = false; creativeState[tool].source = ''; creativeState[tool].generating = false; creativeState[tool].pendingRisks = false; creativeState[tool].messages = creativeHistory.filter((item) => item.tool === tool); creativeMessages(tool).innerHTML = ''; });

...
function renderCreativeResult(node, tool, result) {
  ...
  addCreativeCopyBlock(node, '可直接复制的排版文本', '仅调整换行、留白与表情', result.formattedText || '');
...
async function generateContentPlan() {
  ...
  try {
    ...
    renderPlanCard(card, { eyebrow: `内容规划 V${version} 已完成`, title: '你的专属内容规划方案', subtitle: '已梳理保险主线、泛内容支线与内容方向。', buttonText: '查看内容规划', onView: () => renderContentPlan(result.plan, version) }); if (state.matched) persistPlanningMessage('assistant', `内容规划 V${version} 已生成。`); refreshContentPlanButton();
...
function completeProfile() {
  state.done = true; setChips(null); setInputMode(null); $('chat-title').textContent = '资料已齐，可以直接生成方案'; $('chat-input').placeholder = '可继续补充、修改，或直接说“发我方案”…';
...
async function generateProposal() {
  ...
  try {
    ...
    renderPlanCard(card, { eyebrow: `专属 IP 方案 V${version} 已完成`, title: '你的专属 IP 方案', subtitle: '昵称、平台简介与合规提示已经整理好。', buttonText: '查看 IP 方案', onView: () => renderProposal(result.proposal, version) }); if (state.matched) persistMessage('assistant', `IP 方案 V${version} 已生成。`); refreshProposalButton();
  } catch (error) { card.textContent = `生成失败：${error.message}`; }

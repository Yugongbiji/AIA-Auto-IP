...
const state = { matched: false, profile: {}, currentQuestion: 0, done: false, version: 1, pendingUpdate: null, multiSelection: new Set(), hasHistory: false, proposals: [], generating: false, autoGenerated: false, activeTool: 'ip', requestedTool: null };
const planningState = { context: {}, currentQuestion: 0, done: false, started: false, hasHistory: false, messages: [], plans: [], version: 1, generating: false, multiSelection: new Set(), suggestions: [] };
const creativeState = {
  script: { started: false, messages: [], source: '', generating: false, pendingRevision: '' },
  xhs: { started: false, messages: [], source: '', generating: false, pendingRisks: false },
};
...
const planningQuestions = [
  { key: 'primaryGoal', ask: '这个账号接下来最想解决什么？先确定主目标，后面的合集才不会东一块、西一块。', chips: ['拓客为主', '增员为主', '两者兼顾'] },
  { key: 'insuranceFocus', ask: '保险这条主线，你更希望重点讲哪些方向？可多选，也可以手动补充；我会在最终方案里帮你收束成清晰的保险主线。', chips: ['家庭保障', '教育金规划', '养老规划', '健康保障', '财富保障', '企业主保障'], multiple: true },
  { key: 'secondaryMaterials', ask: '下面是根据你的报名资料、已填写 IP 信息和已有 IP 方案做出的初步候选。可多选，也可以补充真实积累；最终只会为账号确认一个最合适的“保险 + 1”。', chips: [], multiple: true },
];
...
function selectTool(tool) {
  ...
  const isXhs = tool === 'xhs';
  $('ip-chat-panel').classList.toggle('hidden', !isIp);
  $('planning-panel').classList.toggle('hidden', !isPlanning);
  $('script-panel').classList.toggle('hidden', !isScript);
  $('xhs-panel').classList.toggle('hidden', !isXhs);
  ...
  if (isScript) activateCreativeTool('script');
  if (isXhs) activateCreativeTool('xhs');
}

function updateWorkspaceHeadings() {
  const hasIpPlan = state.proposals.length > 0;
  const hasContentPlan = planningState.plans.length > 0;
  $('chat-subtitle').textContent = hasIpPlan
    ? '你的定位已确定；保持稳定，必要时再微调。'
    : state.done ? '资料已齐，正在为你打造长期可用的人设。' : '从真实资料中，找到更像你的长期定位。';
  $('planning-subtitle').textContent = hasContentPlan
    ? '内容方向已确定；下一步把选题交给智能脚本改写。'
    : hasIpPlan ? '结合你的 IP，确定稳定、聚焦的内容方向。' : '先完成 IP 人设，再规划更准确的内容方向。';
  $('script-subtitle').textContent = '结合您的IP，智能脚本改写！';
  $('xhs-subtitle').textContent = '不改一个字，让每一个重点更容易被看见。';
}
...
function setPlanningChips(question) {
  ...
  question.chips.forEach((chip) => {
    ...
    if (question.multiple) {
      button.classList.toggle('selected', planningState.multiSelection.has(chip));
      button.onclick = () => togglePlanningOption(chip, question);
    } else button.onclick = () => planningAnswer(chip);
    area.appendChild(button);
  });
  if (!question.multiple) return;
  Array.from(planningState.multiSelection).filter((value) => !question.chips.includes(value)).forEach((value) => {
    const selected = document.createElement('button'); selected.className = 'selected custom-selected'; selected.textContent = `${value} ×`;
    selected.onclick = () => togglePlanningOption(value, question); area.appendChild(selected);
  });
  const custom = document.createElement('div'); custom.className = 'custom-multi-input';
  const customInput = document.createElement('input'); customInput.placeholder = question.key === 'insuranceFocus' ? '补充保险细分方向…' : '补充你的真实积累…';
  const add = document.createElement('button'); add.type = 'button'; add.textContent = '添加';
  const addCustom = () => { const value = customInput.value.trim(); if (!value) return; planningState.multiSelection.add(value); customInput.value = ''; setPlanningChips(question); };
  add.onclick = addCustom; customInput.onkeydown = (event) => { if (event.key === 'Enter') { event.preventDefault(); addCustom(); } };
  custom.append(customInput, add); area.appendChild(custom);
  const confirm = document.createElement('button'); confirm.className = 'primary multi-confirm'; confirm.textContent = `确认已选（${planningState.multiSelection.size}）`; confirm.disabled = planningState.multiSelection.size === 0;
  confirm.onclick = () => confirmPlanningMulti(question); area.appendChild(confirm);
}

function togglePlanningOption(value, question) {
  if (planningState.multiSelection.has(value)) planningState.multiSelection.delete(value); else planningState.multiSelection.add(value);
  setPlanningChips(question);
}

function confirmPlanningMulti(question) {
  const value = Array.from(planningState.multiSelection).join('、'); if (!value) return;
  addPlanningMessage(value, 'user'); planningState.context[question.key] = value; planningState.multiSelection = new Set(); planningState.currentQuestion += 1;
  setPlanningChips(null); presentPlanningQuestion();
}

function planningEvidenceText() {
  const proposal = state.proposals[0]?.proposal || {};
  return [
    state.profile.customerGroups, state.profile.customerAges, state.profile.selfIntro, state.profile.purpose,
    state.profile.strengths, state.profile.honors, state.profile.education, state.profile.schoolTier,
    proposal.headline, proposal.subheadline, proposal.clientPortrait?.text, ...(proposal.tags || []),
    ...(proposal.advantages || []).map((item) => `${item.title || ''} ${item.text || ''}`),
  ].filter(Boolean).join(' ');
}

function planningPayload() {
  const proposal = state.proposals[0]?.proposal;
  if (!proposal) return { ...planningState.context };
  return {
    ...planningState.context,
    ipPlanReference: {
      headline: proposal.headline || '', subheadline: proposal.subheadline || '', tags: proposal.tags || [],
      clientPortrait: proposal.clientPortrait?.text || '', advantages: (proposal.advantages || []).map((item) => `${item.title || ''} ${item.text || ''}`),
    },
  };
}

function personalizedSecondarySuggestions() {
  const evidence = planningEvidenceText(); const suggestions = [];
  const add = (label, reason) => { if (!suggestions.some((item) => item.label === label)) suggestions.push({ label, reason }); };
  if (/宝爸宝妈|育儿|孩子|教育|升学|教育金/.test(evidence)) add('育儿与升学', '人群：已出现宝爸宝妈、孩子或教育相关线索；积累：需有真实育儿/教育观察；价值：帮助家庭解决高频成长规划问题。');
  if (/企业主|创业|经营|老板|企业/.test(evidence)) add('企业经营与家庭财富', '人群：企业主或经营者线索较明确；积累：需有企业经营或服务经验；价值：围绕企业主家庭的风险与传承场景做利他科普。');
  if (/职场|白领|职业|增员|团队|转型|学历|博士|硕士/.test(evidence)) add('职场成长与职业选择', '人群：职场白领、准增员或职业转型线索匹配；积累：可结合真实职业经历；价值：帮助受众理解职业选择与成长路径。');
  if (/银发|养老|退休|55|健康|医疗/.test(evidence)) add('银发生活与养老准备', '人群：都市银发、养老或健康保障线索匹配；积累：需有真实服务观察；价值：把养老准备讲得更清楚、更生活化。');
  if (/运动|骑行|跑步|健身|高尔夫|户外/.test(evidence)) add('高质量运动生活', '人群：已有运动经历线索；积累：需能长期真实分享；价值：优先服务与目标客户画像一致的运动圈层，不建议泛泛追热点。');
  if (/女性|妈妈|母婴/.test(evidence)) add('家庭角色与女性成长', '人群：需与目标女性客户或宝妈画像一致；积累：必须来自真实生活与服务经验；价值：围绕家庭角色中的实际难题提供帮助。');
  if (!suggestions.length) add('从真实经历中筛选第二主线', '目前可用于判断的个人线索不够多。请补充你长期投入的经历、兴趣或服务场景；系统会按人群一致、可持续、利他三项筛选。');
  return suggestions.slice(0, 4);
}

function addSecondarySuggestionGuide(suggestions) {
  const node = document.createElement('div'); node.className = 'message assistant planning-suggestion-guide';
  node.append(makeNode('strong', '', '我先按“三原则”筛了一轮候选：'));
  const intro = makeNode('p', '', '判断标准：是否吸引准客户或准增员、你能否持续输出、能否长期利他。每个候选都只是初步参考；最终账号只能确定一个“保险 + 1”。'); node.append(intro);
  const list = document.createElement('div'); list.className = 'planning-suggestion-list';
  suggestions.forEach((item) => { const card = document.createElement('button'); card.type = 'button'; card.className = 'planning-suggestion-card'; card.append(makeNode('b', '', item.label), makeNode('span', '', item.reason)); card.onclick = () => { planningState.multiSelection.add(item.label); setPlanningChips({ ...planningQuestions[planningState.currentQuestion], chips: suggestions.map((entry) => entry.label) }); }; list.appendChild(card); });
  node.append(list); planningMessages().appendChild(node); planningMessages().scrollTop = planningMessages().scrollHeight;
}
...
function planningReadyPrompt() {
  ...
  words.textContent = '关键素材已经收到。现在可以按“保险 + N → 保险 + 1”做第一版内容规划。';
  const button = document.createElement('button');
...
function renderPlanCard(card, { eyebrow, title, subtitle, buttonText, onView }) {
...
function creativeMessages(tool) { return $(`${tool}-messages`); }

function creativeSaveState(tool) { return $(`${tool}-save-state`); }

async function persistCreativeMessage(tool, role, content, result = null) {
  if (!state.matched) return;
  try {
    await fetch('/api/creative/message', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ agentId: state.profile.agentId, tool, role, content, result }) });
  } catch (_) { creativeSaveState(tool).textContent = '暂未保存，请检查网络'; }
}

function addCreativeMessage(tool, content, role = 'assistant', persist = true, result = null) {
  const node = document.createElement('div'); node.className = `message ${role}`;
  if (result) renderCreativeResult(node, tool, result); else node.textContent = content;
  creativeMessages(tool).appendChild(node); creativeMessages(tool).scrollTop = creativeMessages(tool).scrollHeight;
  if (persist && state.matched) persistCreativeMessage(tool, role, content, result);
  return node;
}
...
function addCreativeCopyBlock(parent, label, focus, text) {
  ...
  const copy = makeNode('button', 'copy-button', '复制全文'); copy.type = 'button'; copy.onclick = () => copyText(text, copy); heading.append(words, copy);
  const textarea = document.createElement('textarea'); textarea.className = 'creative-textarea'; textarea.value = text; textarea.readOnly = true; textarea.rows = Math.max(6, Math.min(18, text.split('\n').length + 2));
...
function renderCreativeResult(node, tool, result) {
  ...
  if (!risks.length) {
    riskBox.append(makeNode('strong', '', '初步风险检测'), makeNode('p', '', '经过初步检测暂无风险词汇。'));
  } else {
    riskBox.append(makeNode('strong', '', `初步检测到 ${risks.length} 处需留意的表达`));
    risks.forEach((risk) => { const row = makeNode('div', 'risk-row'); row.append(makeNode('b', '', `“${risk.snippet}”`), makeNode('span', '', `${risk.type || '表达风险'}：${risk.reason || '建议发布前核对。'}`), makeNode('small', '', `建议：${risk.suggestion || '确认后可交给脚本改写处理。'}`)); riskBox.append(row); });
    const actions = makeNode('div', 'risk-actions');
    const keep = makeNode('button', 'secondary-button', '保留原文'); keep.type = 'button'; keep.onclick = () => addCreativeMessage('xhs', '已保留当前排版原文。', 'system');
    const revise = makeNode('button', 'primary', '确认修改'); revise.type = 'button'; revise.onclick = () => handoffXhsToScript(result.source || creativeState.xhs.source);
    actions.append(keep, revise); riskBox.append(actions);
  }
  node.append(riskBox);
}

function activateCreativeTool(tool) {
  const toolState = creativeState[tool]; if (toolState.started) return;
  toolState.started = true; creativeSaveState(tool).textContent = state.matched ? '将保存到历史档案' : '本次会话';
  toolState.messages.forEach((item) => {
    if (item.result) renderCreativeResult(addCreativeMessage(tool, '', item.role, false), tool, item.result);
    else addCreativeMessage(tool, item.content, item.role, false);
    if (item.result?.source) toolState.source = item.result.source;
    if (tool === 'xhs' && Array.isArray(item.result?.risks) && item.result.risks.length) toolState.pendingRisks = true;
...
async function runScriptRewrite(source, revision = '') {
  const toolState = creativeState.script; if (toolState.generating) return;
  toolState.generating = true; creativeSaveState('script').textContent = '正在改写…';
  const card = addCreativeMessage('script', '正在保留原文事实、检查合规表达，并整理 3 篇不同角度的改写稿…', 'assistant', false);
  try {
    const response = await fetch('/api/script/rewrite', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ profile: state.profile, ipPlan: latestIpPlanReference(), source, revision }) });
    const result = await response.json(); if (!response.ok) throw new Error(result.error || '改写失败');
    const saved = { ...result, source }; renderCreativeResult(card, 'script', saved); persistCreativeMessage('script', 'assistant', '脚本改写已完成。', saved);
  } catch (error) { card.textContent = `改写失败：${error.message}`; }
  finally { toolState.generating = false; creativeSaveState('script').textContent = state.matched ? '已保存到历史档案' : '本次会话'; }
}

async function runXhsFormat(source, revision = '') {
  const toolState = creativeState.xhs; if (toolState.generating) return;
  toolState.generating = true; creativeSaveState('xhs').textContent = '正在排版并检测…';
  const card = addCreativeMessage('xhs', '正在整理手机阅读节奏，并做初步风险检测。原文不会被改动。', 'assistant', false);
  try {
    const response = await fetch('/api/xhs/format', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ profile: state.profile, source, revision }) });
    const result = await response.json(); if (!response.ok) throw new Error(result.error || '排版失败');
    const saved = { ...result, source }; toolState.pendingRisks = (saved.risks || []).length > 0; renderCreativeResult(card, 'xhs', saved); persistCreativeMessage('xhs', 'assistant', '小红书排版已完成。', saved);
  } catch (error) { card.textContent = `排版失败：${error.message}`; }
  finally { toolState.generating = false; creativeSaveState('xhs').textContent = state.matched ? '已保存到历史档案' : '本次会话'; }
}

function handoffXhsToScript(source, confirmation = '') {
  if (!source) return;
  if (confirmation) addCreativeMessage('xhs', '已收到确认，正在修改风险表达并转入脚本改写。', 'system');
  else addCreativeMessage('xhs', '已确认修改风险表达，正在转入脚本改写。', 'user');
  creativeState.xhs.pendingRisks = false;
  creativeState.script.source = source; selectTool('script');
  addCreativeMessage('script', '这份原文已从小红书排版的风险提示转入脚本改写。我会只修改需要处理的风险表达，并保留原文核心观点。', 'system');
  return runScriptRewrite(source, '请根据小红书排版后的风险提示，修改有风险的表达；保留原文核心观点与事实。');
}

function isExplicitRevision(text, includeFormat = false) {
  const normalized = text.replace(/\s+/g, '');
  const action = includeFormat ? '(?:修改|改写|重写|调整|处理|优化|排版|重排)' : '(?:修改|改写|重写|调整|处理|优化)';
  return new RegExp(`(?:确认|同意|可以|确定|按建议|按提示|按风险|按这个|按这些|按前面|就按).{0,10}${action}|(?:重新|再).{0,8}${action}|^(?:改吧|改一下|按这个来|就这样改|开始修改|开始改写)$`).test(normalized);
}

function isXhsConfirmRevision(text) { return isExplicitRevision(text, true); }
function isScriptConfirmRevision(text) { return isExplicitRevision(text, false); }
...
function fallbackScriptIntent(text) {
  ...
  if (isScriptConfirmRevision(text)) return { intent: 'rewrite_request', reply: '收到，正在按已确认的意见重新改写当前脚本。', revisionInstruction: text };
...
async function handleScriptConversation(content) {
  ...
  if (isScriptConfirmRevision(content)) {
    const instruction = toolState.pendingRevision || content;
    toolState.pendingRevision = '';
    addCreativeMessage('script', '收到确认，正在按这些意见重新改写当前脚本。', 'system');
    return runScriptRewrite(toolState.source, instruction);
  }
  ...
  if (intent === 'rewrite_request') {
    const instruction = [toolState.pendingRevision, result.revisionInstruction || content].filter(Boolean).join('；');
    toolState.pendingRevision = '';
    addCreativeMessage('script', reply, 'system');
    return runScriptRewrite(toolState.source, instruction);
  }
  if (intent === 'feedback') toolState.pendingRevision = [toolState.pendingRevision, content].filter(Boolean).join('；').slice(0, 600);
...
async function submitCreativeTool(tool, value) {
  ...
  if (tool === 'xhs' && toolState.source && toolState.pendingRisks && isXhsConfirmRevision(content)) {
    addCreativeMessage('xhs', content, 'user');
    return handoffXhsToScript(toolState.source, content);
  }
...
function presentPlanningQuestion() {
  ...
  if (!question) {
    ...
    $('planning-subtitle').textContent = '信息已齐，生成方案后就可以进入智能脚本改写。';
  ...
  $('planning-subtitle').textContent = '用少量关键问题，确定稳定、聚焦的 1 + 1 内容方向。';
  $('planning-input').placeholder = question.multiple ? '也可以手动输入后添加…' : '输入你的回答…';
  planningState.multiSelection = new Set();
  if (question.key === 'secondaryMaterials') {
    planningState.suggestions = personalizedSecondarySuggestions();
    addSecondarySuggestionGuide(planningState.suggestions);
    const resolved = { ...question, chips: planningState.suggestions.map((item) => item.label) };
    addPlanningMessage(question.ask); setPlanningChips(resolved); return;
  }
...
function activatePlanning() {
  ...
  if (planningState.plans.length) {
    ...
    if (!planningState.messages.length) addPlanningMessage('你已经有一版内容规划了。直接告诉我想调整什么，例如“第二主线更偏育儿”或“合集增加增员内容”，我会生成新版本。');
    refreshContentPlanButton(); return;
  }
  addPlanningMessage(state.profile.name ? `${state.profile.name}，IP 资料会自动带入。我们再补四个和内容方向有关的问题，就能生成内容规划。` : 'IP 资料会自动带入。我们再补四个和内容方向有关的问题，就能生成内容规划。');
...
async function planningAnswer(value) {
  ...
  const question = planningQuestions[planningState.currentQuestion];
  if (question.multiple) {
    planningState.multiSelection.add(content);
    const resolved = question.key === 'secondaryMaterials' ? { ...question, chips: planningState.suggestions.map((item) => item.label) } : question;
    setPlanningChips(resolved); return;
  }
...
async function generateContentPlan() {
  ...
  const card = addPlanningMessage('正在梳理保险主线、泛内容候选和合集结构，马上给你第一版。', 'assistant', false);
  try {
    ...
    renderPlanCard(card, { eyebrow: `内容规划 V${version} 已完成`, title: '你的专属内容规划方案', subtitle: '已梳理保险主线、泛内容支线与内容方向。', buttonText: '查看内容规划', onView: () => renderContentPlan(result.plan, version) }); if (state.matched) persistPlanningMessage('assistant', `内容规划 V${version} 已生成。`); refreshContentPlanButton(); updateWorkspaceHeadings();
    if (state.proposals.length) setTimeout(() => selectTool('script'), 650);
...
async function reviseContentPlan(revision) {
  ...
  const card = addPlanningMessage('收到。我会保留合理部分，再按你的补充重新整理成一个可执行的新版本。', 'assistant', false);
  try {
    ...
    renderPlanCard(card, { eyebrow: `内容规划 V${version} 已更新`, title: '你的专属内容规划方案', subtitle: '已按你的补充保留合理部分并重新整理。', buttonText: '查看新版本', onView: () => renderContentPlan(result.plan, version) }); if (state.matched) persistPlanningMessage('assistant', `内容规划 V${version} 已按对话调整。`); refreshContentPlanButton();
...
async function applyPendingUpdate(node) {
  ...
  const notice = addMessage('档案已经换成这版啦。接下来重新生成方案，就能看到新效果。', 'system');
...
function isGenerateRequest(content) {
  ...
  return /(?:生成|重新生成|出|做|发|给|看|展示|打开).{0,12}(?:一版|我的|个|下|最新)?(?:IP)?(?:方案|人设|定位)/.test(text)
    || /(?:IP)?(?:方案|人设|定位).{0,12}(?:生成|重新生成|出|做|发|给|看|展示|打开)/.test(text)
    || /^(?:方案呢|方案|我的方案|发方案|发我|给方案|直接给我|直接发我|发我方案|给我方案|看方案|出方案吧|生成吧|好了吗|没有补充了?|没补充了?|就这样|太烦了|准确|没问题|确认|对|是)$/.test(text);
}
...
async function answer(value) {
  ...
  if (state.done) {
    ...
    try {
      ...
      addMessage('这句话我还没完全拿准，不过没关系。换个说法告诉我想改什么，我会先复述给你确认。', 'assistant');
    } finally {
...
function completeProfile() {
  state.done = true; setChips(null); setInputMode(null); updateWorkspaceHeadings(); $('chat-input').placeholder = '可继续补充、修改，或直接说“发我方案”…';
  if (!state.hasHistory) addMessage('资料已整理完成，正在自动生成你的专属 IP 方案。', 'system');
  renderProfile();
...
function startWorkspace(profile, matched, history = [], proposals = [], planningHistory = [], contentPlans = [], creativeHistory = []) {
...
if (OPERATOR_MODE) $('switch-account').classList.remove('hidden');
...
async function generateProposal() {
  ...
  const button = $('generate-button'); button.disabled = true; button.textContent = '正在生成…'; const card = addMessage('正在生成你的专属 IP 方案，稍等一下。', 'assistant', false);
  try {
    ...
    renderPlanCard(card, { eyebrow: `专属 IP 方案 V${version} 已完成`, title: '你的专属 IP 方案', subtitle: '昵称、平台简介与合规提示已经整理好。', buttonText: '查看 IP 方案', onView: () => renderProposal(result.proposal, version) }); if (state.matched) persistMessage('assistant', `IP 方案 V${version} 已生成。`); refreshProposalButton(); updateWorkspaceHeadings();
  } catch (error) { card.textContent = `生成失败：${error.message}`; }

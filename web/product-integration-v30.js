...
(function () {
  ...
  function isMissingText(value) {
    const normalized = text(value).replace(/[\s。！!？?、，,；;：:·._-]+/g, '');
    return !normalized || /^(无|没有|暂无|无昵称|未填|未填写|未设置|未提供|不知道|不详|NA|N\/A|null|none)$/i.test(normalized);
  }
  function cleanNicknameValue(value) { return isMissingText(value) ? '' : text(value); }
  function normalizeNicknamePlaceholders(profile) {
    if (!profile || typeof profile !== 'object') return profile;
    ['videoNickname', 'xiaohongshuNickname'].forEach((key) => {
      if (isMissingText(profile[key])) profile[key] = '';
    });
    return profile;
  }
  ...
  async function enrichProfileBeforeWorkspace(profile) {
    normalizeNicknamePlaceholders(profile);
  ...
  function evaluateExistingNickname(name,profile){
    const value=cleanNicknameValue(name),hardIssues=[],observations=[],strengths=[];
    if(!value)return{name:'',hardIssues:[],observations:[],strengths:[],recommendation:'暂无昵称',missing:true};
    ...
    let recommendation='建议保留';if(hardIssues.length)recommendation='建议认真考虑调整';else if(!strengths.length)recommendation='建议结合既有使用历史再判断，不宜贸然修改';return{name:value,hardIssues,observations,strengths,recommendation,missing:false};
  }
  ...
  function renderNicknameAdvice(){
    normalizeNicknamePlaceholders(state.profile||{});
  ...
  startWorkspace=function integratedStartWorkspace(profile,...rest){normalizeNicknamePlaceholders(profile);applyAudienceQuestionContext();enrichProfileBeforeWorkspace(profile).then((enriched)=>{normalizeNicknamePlaceholders(enriched);applyAudienceQuestionContext();baseStartWorkspace(enriched,...rest);});};
  const basePresentQuestion=presentQuestion;
  presentQuestion=function integratedPresentQuestion(){normalizeNicknamePlaceholders(state.profile||{});applyAudienceQuestionContext();return basePresentQuestion();};
  const baseRenderProposal=renderProposal;
  renderProposal=function integratedRenderProposal(proposal,version){normalizeNicknamePlaceholders(state.profile||{});scrubUnsupportedSchoolClaims(proposal,state.profile||{});const result=baseRenderProposal(proposal,version);requestAnimationFrame(()=>requestAnimationFrame(renderNicknameAdvice));return result;};
  ...
  window.aiaProductIntegrationV30={applySemanticUpdates,enrichProfileBeforeWorkspace,applyAudienceQuestionContext,relabelFloatingActions,evaluateExistingNickname,distinctiveAssets,renderNicknameAdvice,scrubUnsupportedSchoolClaims,isMissingText,normalizeNicknamePlaceholders};
})();

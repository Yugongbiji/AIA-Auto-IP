...
(function(){
  ...
  const BIO_MIN=12;
  function lineWeight(value){return [...t(value)].reduce((n,ch)=>/\p{Extended_Pictographic}/u.test(ch)?n+2:/[\u0000-\u007f]/.test(ch)?n+0.6:n+1,0);}
  function visibleBioText(value){return t(value).replace(/^\p{Extended_Pictographic}(?:\uFE0F)?\s*/u,'').trim();}
  function isFixedFooter(line){const v=visibleBioText(line);return /^(本账号所述内容为个人意见|本账号上所陈述或表达的内容仅为我个人意见|营销服务部：|执业证编号：)/.test(v);}
  ...
  function simpleModal(title,lines,confirm,onConfirm){const back=document.createElement('div');back.className='copy-reminder-backdrop';back.setAttribute('role','dialog');back.setAttribute('aria-modal','true');const card=document.createElement('section');card.className='copy-reminder-modal';const h=document.createElement('h3');h.textContent=title;card.appendChild(h);const ul=document.createElement('ul');lines.forEach(text=>{const li=document.createElement('li');li.textContent=text;ul.appendChild(li);});card.appendChild(ul);const actions=document.createElement('div');actions.className='copy-reminder-actions';const ok=document.createElement('button');ok.type='button';ok.className='primary';ok.textContent=confirm;ok.onclick=()=>{close(back);onConfirm?.();};actions.appendChild(ok);card.appendChild(actions);back.appendChild(card);document.body.appendChild(back);document.body.classList.add('copy-reminder-open');ok.focus();}
  function complianceModal(kind,text,button,copyAfter=true){const cfg=COMPLIANCE[kind]||COMPLIANCE.bio;const back=document.createElement('div');back.className='copy-reminder-backdrop';back.setAttribute('role','dialog');back.setAttribute('aria-modal','true');const card=document.createElement('section');card.className='copy-reminder-modal aia-compliance-modal';const h=document.createElement('h3');h.textContent=kind==='nickname'?'昵称合规提示 ⚠️':'简介合规提示 ⚠️';card.appendChild(h);const grid=document.createElement('div');grid.className='aia-compliance-grid';[['可以说',cfg.can,'aia-compliance-can'],['不可以说',cfg.cannot,'aia-compliance-cannot']].forEach(([title,items,cls])=>{const col=document.createElement('section');col.className=`aia-compliance-column ${cls}`;const hh=document.createElement('h4');hh.textContent=title;const ul=document.createElement('ul');items.forEach(x=>{const li=document.createElement('li');li.textContent=x;ul.appendChild(li);});col.append(hh,ul);grid.appendChild(col);});card.appendChild(grid);const actions=document.createElement('div');actions.className='copy-reminder-actions';const ok=document.createElement('button');ok.type='button';ok.className='primary';ok.textContent=copyAfter?'我已了解，继续复制':'我知道了';ok.onclick=async()=>{close(back);if(copyAfter){seen[kind]=true;await write(text,button);}};actions.appendChild(ok);card.appendChild(actions);back.appendChild(card);document.body.appendChild(back);document.body.classList.add('copy-reminder-open');ok.focus();}
  ...
  function help(content,kind){
    ...
    if(!heading)return;
    const card=heading.closest('.proposal-card,section,article')||heading.parentElement;
    if(!card||card.querySelector(`.aia-compliance-help[data-kind="${kind}"]`))return;
    card.classList.add('aia-compliance-help-host');
  ...
  function sanitizeBioBlocks(content){
    content.querySelectorAll('.bio-copy-block').forEach(block=>{
      const textarea=block.querySelector('textarea');if(!textarea)return;
      const lines=t(textarea.value).split('\n').map(t).filter(Boolean);
      // 120/124：固定合规尾部不参与字数门禁；正文/总结句低于最低信息量时不得单独占一行。
      const safe=lines.filter(line=>isFixedFooter(line)||lineWeight(visibleBioText(line))>=BIO_MIN);
      textarea.value=safe.join('\n');textarea.rows=Math.max(4,safe.length);
    });
  }
  function normalizeBioRecommendationUi(content){
  ...
  if(typeof renderProposal==='function'){const base=renderProposal;renderProposal=function productRulesV10ComplianceOnly(proposal,version){const result=base(proposal,version);const content=document.getElementById('proposal-content');if(content)enhance(content,proposal);return result;};}
